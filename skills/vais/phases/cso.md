...
`agents/cso/cso.md`를 읽고 그 안의 지침에 따라 실행하세요.
...
- **Phase 명시**: `/vais cso plan my-feature` → phase=`plan`, feature=`my-feature`
- **Phase 생략**: `/vais cso my-feature` → phase=미지정, feature=`my-feature`
...
- **options**:
  ...
  - `현재 C-Level 다음 phase` — `/vais cso {다음phase} {feature}`
  - `종료` — 작업 종료
...
- `현재 C-Level 다음 phase` → `skills/vais/phases/cso.md` Read → `{다음phase}` 로 실행
